...
const Categories = () => {
    ...
    return (
        <div className='w-full flex flex-col items-center gap-4 py-5 min-h-screen'>
            <h1 className='text-base md:text-2xl font-semibold text-center flex items-center justify-center gap-4 flex-row'>Our top categories <MdStar/></h1>
            <div className='w-full grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5  gap-10'>
            
            {categories.map((cat) => (
                <motion.div initial={{opacity:0, scale:0.9, rotate:6}} whileInView={{opacity:1, scale:1, rotate:0}} transition={{duration:0.7}} key={cat._id} className=' gap-2 group shadow cursor-pointer flex items-center flex-col p-2 rounded-full'>
