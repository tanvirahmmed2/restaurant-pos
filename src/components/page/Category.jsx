...
const Category = () => {
    ...
    return (
        <div className='w-full grid grid-cols-2 md:grid-cols-4  gap-10'>
            {categories.map((cat) => (
                <motion.div initial={{opacity:0, scale:0.9, rotate:6}} whileInView={{opacity:1, scale:1, rotate:0}} transition={{duration:0.7}} key={cat._id} className='bg-slate-100 gap-2 group shadow cursor-pointer flex items-center flex-col p-2 rounded-2xl'>
                    ...
                    <Link href={`/category/${cat.slug}`} className='text-xl font-mono font-semibold'>{cat?.name}</Link>
                </motion.div>
            ))}
        </div>
    )
